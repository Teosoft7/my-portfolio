...
export default function SubProjects() {
  ...
  const subProjects = allProjects.filter((p) => !p.featured);
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-black/40 flex items-center justify-center border border-white/5">
                  {item.type === "Article" ? <BookOpen className="w-5 h-5 text-purple-400" /> : <Code className="w-5 h-5 text-blue-400" />}
                </div>
                <span className="text-xs font-semibold uppercase tracking-wider text-neutral-500">{item.type || "Project"}</span>
              </div>
